...
func modules() {
	...
	case 1:
		...
		search(1)
	case 2:
		fmt.Println("not working currently")
		os.Exit(1)
		search(2)
	default:
		os.Exit(1)
...
func flipping(ri io.ReadCloser) {
	...
	for i, _ := range goods.Data.Items {
		...
		sellnum := float64(goods.Data.Items[i].SellNum)
		if sellnum > sellorders && (sell*0.975-buy)/buy*100 > percentage && sell > minprice {
...
func validation() {
	sellorders, _ = strconv.ParseFloat(i_reader("enter the minimum selloders [90]:  "), 64)
	if sellorders == 0 {
		sellorders = 90
	}
	minprice, _ = strconv.ParseFloat(i_reader(("enter the minimum price [30]:  ")), 64)
	if minprice == 0 {
		minprice = 30
	}
	percentage, _ = strconv.ParseFloat(i_reader("enter the percentual difference of buy order and sell order [4]:  "), 64)
	if percentage == 0 {
		percentage = 4
	}
}
